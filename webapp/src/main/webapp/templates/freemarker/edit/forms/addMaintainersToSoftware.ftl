...
<#-- Custom form for adding maintainers to information resources -->

<#import "lib-vivo-form.ftl" as lvf>

<#--Retrieve certain page specific information information-->
<#assign newRank = editConfiguration.pageData.newRank />
<#assign existingMaintainerInfo = editConfiguration.pageData.existingMaintainerInfo />
...
<#--UL class based on size of existing maintainers-->
<#assign ulClass = ""/>
<#if (existingMaintainerInfo?size > 0)>
...
<h3>${i18n().manage_maintainers}</h3>

<ul id="dragDropList" ${ulClass}>

<script type="text/javascript">
    var maintainershipData = [];
</script>


<#assign maintainerHref="/individual?uri=" />
<#--This should be a list of java objects where URI and name can be retrieved-->
<#list existingMaintainerInfo as maintainership>
	<#assign maintainerUri = maintainership.maintainerUri/>
	<#assign maintainerName = maintainership.maintainerName/>

	<li class="maintainership">
			<#-- span.maintainer will be used in the next phase, when we display a message that the maintainer has been
			removed. That text will replace the a.maintainerName, which will be removed. -->
			<span class="maintainer">
					<#-- This span is here to assign a width to. We can't assign directly to the a.maintainerName,
					for the case when it's followed by an em tag - we want the width to apply to the whole thing. -->
					<span class="itemName">
							<#if (maintainerUri?length > 0)>
									<span class="maintainerName">${maintainerName}</span>
								<#else>
									<span class="maintainerName">${maintainership.maintainershipName}</span><em> (${i18n().no_linked_maintainer})</em>
							</#if>
					</span>

					<a href="${urls.base}/edit/primitiveDelete" class="remove" title="${i18n().remove_maintainer_link}">${i18n().remove_capitalized}</a>
			</span>
	</li>

	<script type="text/javascript">
                        maintainershipData.push({
					"maintainershipUri": "${maintainership.maintainershipUri}",
					"maintainerUri": "${maintainerUri}",
...
<section id="showAddForm" role="region">
    ...
    <input type="submit" id="showAddFormButton" value="${i18n().add_maintainer}" role="button" />

    <span class="or"> ${i18n().or} </span>
    <a id="returnLink" class="cancel" href="${cancelUrl}&url=/individual" title="${i18n().cancel_title}">${i18n().return_to_software}</a>
    <img id="indicatorOne" class="indicator hidden" alt="${i18n().processing_indicator}" src="${urls.base}/images/indicatorWhite.gif" />
</section>

<form id="addMaintainerForm" action ="${submitUrl}" class="customForm noIE67">
    <h3>${i18n().add_a_maintainer}</h3>
    ...
    <section id="personFields" role="personContainer">
        ...
        <div id="selectedMaintainer" class="acSelection">
            <p class="inline">
                <label>${i18n().selected_maintainer}:&nbsp;</label>
                <span class="acSelectionInfo" id="selectedMaintainerName"></span>
    ...
    <input type="hidden" id="label" name="label" value=""  role="input" />  <!-- Field value populated by JavaScript -->
        ...
        <p class="submit">
            ...
            <input type="submit" id="submit" value="${i18n().add_maintainer}" role="button" role="input" />
...
var i18nStrings = {
    maintainerNameWrapperTitle: '${i18n().drag_drop_reorder_maintainers?js_string}',
    reorderMaintainersAlert: '${i18n().reordering_maintainers_failed?js_string}',
    removeMaintainershipMessage: '${i18n().confirm_maintainer_removal?js_string}',
    removeMaintainershipAlert: '${i18n().error_processing_maintainer_request?js_string}',
    maintainerTypeText: '${i18n().maintainer_capitalized?js_string}',
...
${scripts.add('<script type="text/javascript" src="${urls.base}/templates/freemarker/edit/forms/js/addMaintainersToSoftware.js"></script>')}
